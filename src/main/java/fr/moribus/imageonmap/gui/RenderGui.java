...
public class RenderGui extends ActionGui
{
    ...
    protected void onUpdate()
    {
        ...
        action("toggle_resize", 0, new ItemStackBuilder(Material.PAINTING)
                ...
                .loreSeparator()
                .longLore(ChatColor.GRAY, I.t("You can automatically resize the image to a certain number of blocks (or item frames)."))
                .loreSeparator()
                .loreLine(ChatColor.BLUE, I.t("Original size (in blocks)"))
                .loreLine(ChatColor.GRAY, originalSizeLoaded ? I.t("{0} × {1}", originalWidth, originalHeight) : I.t("Loading..."))
                .loreSeparator()
                .longLore(resize ? I.t("{gray}» {white}Click{gray} to disable resize") : I.t("{gray}» {white}Click{gray} to enable resize"))
        );
    ...
    private ItemStackBuilder getBannerButton(boolean positive, boolean longStep, boolean disabled)
    {
        ...
        return new ItemStackBuilder(TextualBanners.getCharBanner(symbol, background, DyeColor.WHITE));
    }
}
